describe("Test feedback pages", function () {
    it("Index should not be available if not logged in", function () {
        cy.request({ url: "http://localhost:5289/Feedback", failOnStatusCode: false, followRedirect: false }).its("status").should("equal", 302)
    })
    it("Create should not be available if not logged in", function () {
        cy.request({ url: "http://localhost:5289/Feedback/Create", failOnStatusCode: false, followRedirect: false }).its("status").should("equal", 302)
    })
    ...
    it("Create should not be available if the user doesn't have an active team membership", function () {
        ...
        cy.get('.alert').should("be.visible").and("include.text", "Please join a team before trying to submit feedback for colleagues!")
    })
    it("Create should not be available if the relevant team has no other members", function () {
        ...
        cy.get('.alert').should("be.visible").and("include.text", "Sorry, it looks like your colleagues haven't joined this team yet.")
    })
    ...
    it("Create should display validation", function () {
        ...
        cy.get("#TargetUser-error").should("be.visible").and("contain", "The User field is required.")
        cy.get("#FeedbackText-error").should("be.visible").and("contain", "The Feedback field is required.")
    })
    ...
    it("Team member should not be able to access Feedback Details page", function () {
        ...
        cy.get("#Password").type("Test-1234")
        cy.get("input[value='Login']").click()
        cy.request({ url: "http://localhost:5289/Feedback/Details/4", failOnStatusCode: false, followRedirect: false }).its("status").should("equal", 404)
    })
    ...
    it("Team member should not be able to access Feedback Delete page", function () {
        ...
        cy.get("#Password").type("Test-1234")
        cy.get("input[value='Login']").click()
        cy.request({ url: "http://localhost:5289/Feedback/Delete/4", failOnStatusCode: false, followRedirect: false }).its("status").should("equal", 302)
    })
